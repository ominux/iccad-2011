...
Another $H_fO_2$-based bipolar ReRAM was implemented by ITRI this year
...
Although there are several variants of ReRAMs, all of them can be
...
Morever, the control of SET, RESET, and read operations without any
...
structure. In the MOS-accessed memory array, a MOSFET is added as an
...
Also, in order to prove enough driven current, lager than minimum
...
cross-point ReRAM
array~\cite{ReRAM_ISSCC2011_Sheu}~\cite{memristor:IEDM08_3D}.
...
In a cross-point structure, the write operation can either write one bit
...
writing operation to prevent unintentional writing~\cite{memristor:Cong},
...
bitlines can either be left floating or half biased. However, while
reading a cell, the selected wordline should be biased at read voltage and
all other wordlines and bitlines in the array are shunted to ground. The
current in each bitline is then sensed and compared to a reference current
to determine cell content. However, due to the sneak current existing at
the cross-point array, the current in bitline also varies depending upon
the values stored in unselected cells.
...
write operation was proposed: in the first step, the background current of
