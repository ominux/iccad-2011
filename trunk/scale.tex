...
about 35\% of the area of the memory array cells. To design a memory array with an effective cell size close to $4F^2$, we need to make sure the
nonlinearity and write current should satisfy certain conditions so that the driver overhead is less than 100\% and the wordline drivers can be almost ''hidden'' underneath the ReRAM cells. As nonlinearity and write
...
arrays is productive in further reduce the effective cell size to $4/N_l F^2$ where $N_l$ is the number of layers.
...
$I_{on\_opt}$ can be determined. For example, when the array size is fixed at $512 \times 512$ and the minimum noise margin is $50mV$, a cross-point
