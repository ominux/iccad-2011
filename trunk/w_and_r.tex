...
First of all, we use an example to show the inherent problem of FWFB scheme, which may result in severe write disturbance.
...
On the other hand, the non-linearity can also reduce the energy consumption of the cross-point array. Take the $128 \times 128$ array for example. As shown in Figure~\ref{fig:non_linear_energy}, the energy consumption for the write operation decreases dramatically with the increase of non-linearity coefficient $K_r$. As $K_r$ increases from 1 to 40, the write energy is reduced by 98..\%.


The write energy with $K_r=20$ is only $3.6\%$ of the energy consumption
