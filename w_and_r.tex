...
So far, we only discuss the one bit per access write operation. It has been show that, in order to write several bits in a word line at the same time, a two step write operation should be adopted~\cite{memristor:Cong}. There are 4 potential write methods: SET-before-RESET,  RESET-before-SET, ERASE-before-SET and ERASE-before-RESET. First of all, the worst case of the voltage drop is not related to the write methods. As shown in Figure.~\ref{fig:reliable_region}, the reliable size of the cross-point array will further reduced. The maximum array size reduces from $116{\times}116$ to $100{\times}100$. 
