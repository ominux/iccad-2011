...
easily derived from Equation (\ref{equ:KCL1}) and (\ref{equ:KCL2}).
...
different. Therefore, these coefficients related to the edge
conditions should be set to proper value according to the program
schemes. Clearly, the four edges shown in Figure~\ref{fig:modeling}
correspond to different coefficients in $A1$ and $A4$. Take the nodes
at the left edge of the array ($V_{i,1}, 1\leq i \leq n$) as an
example. All of the coefficients related to these nodes located in
$A1$, which can be set as:
