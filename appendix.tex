...
\begin{equation}\label{equ:KCL0}
 {\Sigma}_{I=1}^kI_k=0.
\end{equation}
For the sake of brevity, we assume that the wordline voltage
drivers only are located at the edge of $V_{W1} \sim V_{Wm}$ and bitline
multiplexes or are locate at the edge of $V_{B1} \sim V_{Bn}$. Points locate at
...
$A1$ is a tridiagonal matrix and has nonzero elements only located in the
...
the coefficients related to the edge conditions should be set according
...
coefficients of other edges. The coefficients of nodes at the left edge
...
\item The constant terms $C$ are $2mn{\times}1$ vector.
